             ...
             fluidRow(
               sidebarPanel(width = 12,
                            fluidRow( h3( strong('Anwendung des Models')),
                                      h5 (HTML("<b>1.</b> Bitte geben Sie Ihre geschätzten Kennzahlen ein.")),
                                      h5 (HTML("<b>2.</b> Klicken Sie auf <b>'Download'</b>, damit Ihre Ergebnisse als 
                                               CSV-Datei auf Ihrem Computer oder Laptop heruntergeladen werden..")),
                                      h5 (HTML("<b>3.</b> Bitte senden Sie Ihre CSV-Datei an <b>'s62smoon@uni-bonn.de'</b>. 
            ...
            fluidRow(
               ...
               sidebarPanel(width = 6,
                            ...
                            sliderInput("CO2_H2O_Due",
                                        "C02, Wasser, und Düngung flussiß(€/m2)",
                                        min = 0.1,
                                        max = 5,
                                        value =  c(1.5,2.1),
                                        step=0.1),
                            ...
                            sliderInput("Hummel_Nutzlinge",
                                        "Hummel + Nutzlingen (€/m2)",
                                        min = 0.01,
                                        max = 1,
                                        value =  c(0.05,0.1),
                                        step=0.01),
                            sliderInput("PSM_chem",
                                        "Chemisches Pflanzenschutzmittel(€/m2)",
                                        min = 0.01,
                                        max = 2,
                                        value =  c(0.25,0.35),
                                        step=0.01),
                            sliderInput("Heackseln_Entsorgung",
                                        "Häckseln + Entsorgung (€/m2)",
                                        min = 0.01,
                                        max = 2,
                                        value =  c(0.3, 0.45 ),
                                        step=0.01),
                            ...
                            sliderInput("Arbeit_all",
                                        "allgemeine Arbeit: räumen und reinigen (€/m2)",
                                        min = 1,
                                        max = 20,
                                        value =  c(2,5),
                                        step=0.5),
                            ...
                            sliderInput("Arbeit_pf",
                                        "Arbeit für Pflege: drehen, Blätter, Treibe (€/m2)",
                                        min = 1,
                                        max = 30,
                                        value =  c(6,10),
                                        step=0.5),
...
server <- function(input, output) {
  ...
  plot1 <- reactive({
    decisionSupport::plot_distributions(
      ...
      base_size = 13, 
      x_axis_name = "Vergleichendes NPV-Ergebnis"
    )
  })  
  ...
  plot3 <- reactive({
    decisionSupport::plot_cashflow(
      ...
      x_axis_name = "year",
      y_axis_name = "Jährlicher Cashflow [€/m2]",
